...
parametri.numeDirectorSetDate = numeDirectorSetDate;
...
try
    ...
    save(numeFisierDescriptoriPuternicExempleNegative,'descriptoriExempleputernicNegative','-v7.3');
     disp(['Am salvat descriptorii pentru exemplele puternic negative in fisierul ' numeFisierDescriptoriPuternicExempleNegative]);
end

exempleAntrenare = [descriptoriExemplePozitive;descriptoriExempleNegative descriptoriExemplePuternicNegative]';
 eticheteExempleAntrenare = [ones(size(descriptoriExemplePozitive,1),1); -1*ones(size(descriptoriExempleNegative,1)+size(descriptoriExemplePuternicNegative,1),1)];
 [w, b] = antreneazaClasificator(exempleAntrenare,eticheteExempleAntrenare);
 parametri.w = w;
 parametri.b = b; 
%vizualizare model invatat HOG
...
%deja am facut antrenarea cu exemple puternic negative(schimbam valoarea la
%1)
parametri.antrenareCuExemplePuternicNegative = 1;
